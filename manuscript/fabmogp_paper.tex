...
earthquake modeling for seismic hazard analysis, as the earthquake size is taken to be an
input for hazard analysis, while it is an output of dynamic rupture models and thus it is
difficult in practice to specify a given earthquake size. Our UQ approach would enable use
of dynamic simulations by providing a set of NROY points that are likely to produce an earthquake
of the desired size, ensuring that seismic hazard estimates are physics-based, while still
capturing the underlying uncertainty associated with our simulations. This will enable
...
\begin{itemize}
   ...
   \item \texttt{seistools} https://github.com/egdaub/seistools
...
Docker image in a Github repository with instructions on building and running the simulations is
available at https://github.com/alan-turing-institute/fabmogp\_paper.
The associated tutorial to describe this workflow, the \texttt{mogp\_emulator} code, and
the FabSim3 commands can be found at the \texttt{fabmogp} site https://github.com/edaub/fabmogp/blob/master/Tutorial.rst
}

\aucontribute{EGD designed and implemented the UQ workflow and earthquake simulations. HA and DG
