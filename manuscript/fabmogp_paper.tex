...
estimate parameters for these models directly \cite{calibration}.
This poses a challenge for model calibration, as there
...
of the underlying simulation \cite{terashake,m8}. A major goal of the software libraries
...
In this paper, we describe a number of computational tools designed to implement a comprehensive
...
completely understand the relevant physics for modelling frictional failure. However, while
simulations have increasingly been used to understand ground motions and seismic hazard, a full
...
We then show the results of the simulation
...
on the fault \cite{earthquakedynamics}. The simulation calculates the size of an earthquake (which can be measured from seismic data)
...
for natural faults. Roughness is cut off at wavelengths less than 20 grid points.
...
for FabSim3 \texttt{fabmogp} to automate the various steps in the workflow. A map illustrating
where the different software components reside on local and remote resources is shown in
Fig.~\ref{tubemap}, which also shows where additional components useful for managing workflows
reside. In this illustration, the local resources are shown to the left, while the remote HPC
resources are in the large light gray box, and connections used in our workflow are shown in orange.
...
to the seismic moment. Values range from 35 m km to around 250 m km.
...
of around 0.15, while at lower normal stresses the shear/normal stress must be slightly higher
near 0.2 to
...
\funding{EGD HA, IM and DG have received funding from the European Union Horizon 2020 research and innovation programme under grant agreement No 800925 (VECMA) and 824115 (HiDALGO).}
